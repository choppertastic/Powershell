DECLARE @DataPath VARCHAR(MAX) = 'F:\ppclvvnbasql00\f\SQL_Data'
DECLARE @LogPath VARCHAR(MAX) = 'F:\ppclvvnbasql00\l\SQL_Logs'
...
BEGIN
    ...
    WHILE @@FETCH_STATUS = 0
    BEGIN
        SET @AttachCMD = @AttachCMD + ',(FILENAME = ''' + @DataPath + '\' + REVERSE(SUBSTRING(REVERSE(RTRIM(@ndfFileName)),0,CHARINDEX('\',REVERSE(RTRIM(@ndfFileName))))) + ''')'
        FETCH NEXT FROM ndfFileCur INTO @ndfFileName
    END
